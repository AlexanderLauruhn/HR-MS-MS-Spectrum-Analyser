...
public class GraphicSpectrum extends JPanel {
	...
	public void paintComponent(Graphics g) {
		...
		for (Signal s: getSignals()){
			...
			g.drawLine(horizontalDistance+(int)(s.getMass()*(double)spectrumSize/maxM), verticalDistance,
					horizontalDistance + (int)(s.getMass() * (double)spectrumSize / maxM), verticalDistance - (int)(s.getIntensity() * 200 / maxI) - 5);
	...
	public void compareSpectra() {
		float ppm = 1;
		try {
			ppm = Float.parseFloat(Main.main.getDeviationTextField().getText().replace(',', '.'));
		}
		catch (NumberFormatException ex){
			Main.errorMessage("Please enter a numeric deviation value");
		}
		for (Signal signal : getSignals()) {//compare each signal with each molecule
			double deviation = ppm * signal.getMass() / 1000000;// ppm= part per million
			int i = 0;
			Molecule molecule = Main.main.getAllCompounds().get(i);
			while ((signal.getMass() > molecule.calculateMass() || match(signal,molecule,deviation )) && i < Main.main.getAllCompounds().size() ) {//compare until matches to save runtime (allows several matches)
				if (match(signal,molecule,deviation)) {
				...
				i++;
				if (i < Main.main.getAllCompounds().size()){//
					molecule = Main.main.getAllCompounds().get(i);//get next molecule
				}
			}
		}
		setSpectrumTable();
	...
	public void setSpectrumTable() {
		...
		for (Signal s: getSignals()) {
			...
			Main.main.getModel2().addRow(new Object[]{  //add new Signal to table
	...
	public float generateInterceptionForLegend() {
		float segment=(float)(Constants.defaultSpectrumSize * (double)(maxM / (5 * spectrumSize)));
		int i = 1;
		while(i < segment) {
			segment -= segment % i;
			i *= 10;
	 ...
	 * @return
	 */
	public boolean match(Signal signal, Molecule molecule, double deviation) {
		return (signal.getMass() < molecule.calculateMass() + deviation && signal.getMass() > molecule.calculateMass() - deviation);
	}
